...
Boosting aims to increase the accuracy of models by focusing on the training instances that are harder to predict. It is used to convert weak models (high bias, low variance) into strong models. Models are built sequentially, with each new model trying to correct the errors of the combined ensemble of existing models. The final model's output is typically a weighted sum or a weighted vote of the individual models, where the weights depend on the performance of the individual models. Because models are built sequentially, boosting algorithms are generally more time-consuming than bagging algorithms.
### Example

Gradient Boosted Trees (like XGBoost and LightGBM) and AdaBoost are popular examples of boosting algorithms.
